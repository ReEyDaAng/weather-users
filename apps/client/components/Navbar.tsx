...
export default function Navbar() {
  ...
  const base =
    "px-3 py-2 rounded-xl text-sm font-medium transition text-zinc-900 dark:text-zinc-100";
  ...
  return (
    <nav className="flex items-center gap-2 p-3 border-b border-zinc-200 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/70 backdrop-blur">
      <Link href="/" className={`${base} ${pathname === "/" ? active : hover}`}>
        Home
      </Link>
      <Link
        href="/saved"
        className={`${base} ${pathname === "/saved" ? active : hover}`}
      >
        Saved
      </Link>
